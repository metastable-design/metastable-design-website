// src/create-order.js
// Creates a Razorpay order. Prices live here, server-side — the browser
// only ever sends item IDs, so nobody can edit the cart in devtools and
...
export async function handleCreateOrder(request, env) {
  ...
  const amount = Math.round(totalDollars * 100); // smallest unit — cents, for USD
